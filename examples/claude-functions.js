...
async function main() {
  // Connect to the local Claude Code provider through the unified SDK interface.
  const tool = await connect("claude-code");

  // Check whether Claude Code is ready to accept requests before sending a prompt.
  ...
  if (auth?.authSupported && auth.authenticated === false) {
    ...
    if (
      ...
      throw new Error(
        started?.message ?? "Claude Code authentication could not be started."
      );
    }
  ...
  if (!health) {
    throw new Error("Claude Code is not healthy.");
  }
